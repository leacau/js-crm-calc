...
export function AddUser() {
	...
	const datosFiltrados = datos.filter(
		(dato) => dato.dni === newUser.dni || dato.email === newUser.email
	);
	...
	const handleAddUser = async (e) => {
		...
		if (datosFiltrados.length > 0) {
			setNewError(
				'El contacto ya existe, le pertenece a ' + datosFiltrados[0].owner
			);
			setErrorType('error');
		} else {
	...
	if (loading) {
		return (
			<div className='flex justify-center items-center h-screen'>
				<div className='animate-spin rounded-full h-20 w-20 border-b-4 border-blue-100'></div>
			</div>
		);
	}
	return (
